 ...
 * @since 2.0.0.0
 */
public class PostgreSQLFactory implements StorageFactory<SQLStorageConfiguration> {
  ...
  public StorageProvider create(final SQLStorageConfiguration configuration) {
    ...
    final String url = "jdbc:postgresql://" + configuration.host() + ":" + configuration.port() + "/" + configuration.database();

    //TODO: Create SQLManager
    final SQLManager manager = null;

    return new SQLStorageProvider("postgresql", manager);
  }
}